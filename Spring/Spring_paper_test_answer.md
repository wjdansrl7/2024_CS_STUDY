...
2. 1차캐시, 동일성 보장, 트랜잭션을 지원하는 쓰기 지원, 변경 감지, 지연로딩

# 다대다 해결전략

1. 데이터의 중복이 발생한다.

2. N:M관계를 1:N 1:M으로 풀어줄 수 있는 연관 테이블을 추가해준다.

# @valid 를 사용한  dto검증

1.객체의 제약조건을 검증하도록 지시하는 어노테이션

2. controller

# spring security개념 & 인증처리 과정
...
3. 스프링 시큐리티에서 현재 인증된 사용자를 나타내는 객채
